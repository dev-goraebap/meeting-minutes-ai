...
실사용 후 `.dc.html` 프로토타입과 실제 구현을 다시 대조해 다음을 바로잡았다 — "재검토 트리거"에서
예상한 시나리오다:

- **페이지 컨테이너 폭**: 포팅 당시 임의로 Tailwind 기본값 `max-w-2xl`(672px)을 썼는데, 프로토타입은
  `max-width:980px`였다. `--container-page: 980px` 토큰을 추가하고 `max-w-page`로 전 페이지
  교체.
- **AppNav/페이지 컨텐츠 좌우 정렬 불일치**: AppNav는 패딩을 `max-w-2xl` 박스 *안쪽*에 뒀고,
  페이지 컨테이너들은 패딩을 바깥 `<main>`에 두고 안쪽 박스는 패딩 없이 `mx-auto max-w-*`만 썼다.
  같은 폭이라도 두 중첩 구조가 다르면 뷰포트가 컨테이너 폭보다 넓어지는 순간(거의 항상) 헤더
  콘텐츠가 페이지 콘텐츠보다 24px 더 안쪽에서 시작한다. AppNav를 페이지들과 동일한 중첩 패턴
  (바깥 `px-6`, 안쪽 `mx-auto max-w-page`)으로 통일해 해결.
- **탭 활성 상태 하이라이트 안 보임**: `src/shared/ui/tabs.tsx`가 `data-[selected]:` 변형을 썼는데,
  설치된 `@base-ui/react` 버전의 `Tabs.Tab`은 `data-active`를 내보낸다(`data-selected`는 존재하지
  않음) — `node_modules/@base-ui/react/tabs/tab/TabsTabDataAttributes.js`로 확인. `data-[active]:`로
  수정.
- **모바일 하단 탭바가 너무 낮음**: 아이콘 없이 텍스트만 `py-2.5`로 얹어 실측 높이 ~36px였다.
  프로토타입은 아이콘 21px + 라벨 + `padding:8px`로 훨씬 크다. 아이콘 추가 + 패딩 조정으로
  ~57px로 보정.
- **버튼 텍스트 줄바꿈**: 프로토타입의 전역 리셋은 `button{white-space:nowrap}`인데 우리
  `Button`/`TagFilterChip`엔 없어서, 좁은 flex 행에서 버튼 라벨이 단어 중간에 줄바꿈되는 경우가
  있었다(예: 태그 상세의 "배경 컨텍스트" 헤더 행). 두 컴포넌트에 `whitespace-nowrap` 추가 +
  해당 행들은 `flex-wrap`으로 버튼 그룹 전체가 한 단위로 다음 줄에 내려가도록 함.
