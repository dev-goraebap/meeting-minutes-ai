...
- 실제 프로젝트 회의 녹음 + 프로젝트 산출물 문서(요구사항정의서, 비즈니스 규칙, ADR 등)를 함께 참고하니 도메인 용어 오인식(예: 유사 발음 약어 오인식) 교정과 화자 역할 추정이 가능했음 — 이게 "프로젝트 컨텍스트" 개념의 근거.
...
> **2026-07-12 갱신**: 아래 목록은 초안이며 [0004](0004-design-system-handoff.md)/[0005](0005-routed-modals.md)로
> 대체되었다. 실제 v1 페이지는 `/`, `/meetings/[id]`, `/tags`, `/tags/[id]` + 라우티드 모달
> `/meetings/new`, `/tags/new`, `/search` (태그 목록/상세, 검색, 삭제, 다운로드 포함 — 아래
> "v1 범위 제외"의 일부 항목이 실질적으로 범위에 들어왔다). "Project"는 [0002](0002-frontend-architecture-fsd.md)에서
> "Tag"로 이미 격하됨.
...
- 벡터DB / RAG
- 큐 시스템(BullMQ 등)
- 기존 사내 시스템 통합 — 데모 검증 후 별도 판단
- 알림, 액션아이템 담당자 배정 등 부가 기능

> **2026-07-12 갱신**: [0004](0004-design-system-handoff.md)의 디자인 핸드오프가 검색(풀텍스트),
> 삭제(회의 자유/태그는 참조 시 차단), 다운로드(PDF/Markdown/Text), 태그 색상, 무한 스크롤을
> v1에 명시적으로 포함시켰다 — 이 항목들은 애초에 "제외"로 명시된 적은 없었지만 계획에도 없었다.
> 위 목록(문서 파서/벡터DB/큐/사내 통합/알림·담당자배정)은 여전히 v1 제외로 유효하다.

## 테스트용 오디오 파일

실제 검증에 쓴 원본 파일: 약 34분 21초, 한국어, 3인 대화의 프로젝트 스프린트 회의 녹음.
...
> **2026-07-12 갱신**: 위 6개 항목은 여전히 "핵심 가설 검증"의 최소 합격선이다. 다만 실제 v1
> 구현 범위는 [0004](0004-design-system-handoff.md) 디자인 핸드오프를 따라 검색·삭제·다운로드·
> 태그 목록/상세 UI까지 포함한다 — 이는 핵심 가설과 무관한 부가 완성도이며, 이 DoD를 대체하지
> 않는다.
